...
@pyrogram.Client.on_message(pyrogram.Filters.command(["kitabu"]))
async def kitabu_kitukufu(bot, update):
    # logger.info(update)
    TRChatBase(update.from_user.id, update.text, "/kitabu")
    await bot.send_message(
        chat_id=update.chat.id,
        text=Translation.KITABU_KITUKUFU,
        reply_to_message_id=update.message_id
    )
